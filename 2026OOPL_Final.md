...
- **碰撞系統的「櫃檯分發」機制（Facade Pattern）**：
  一開始的碰撞系統簡直是個巨大怪獸，所有地形、玩家、怪物之間的碰撞交織在一起，修 A 壞 B 是家常便飯。重構時，我把它降格成一個**分派櫃檯**，底下開了四個專業小幫手（`PlayerBlockHandler`、`PlayerEntityHandler` 等）各司其職。現在玩家撞怪物的 bug，就只去 `PlayerEntity` 的檔案裡修，完全不用擔心弄壞地形碰撞。各司其職的感覺，讓 Debug 的速度快了十倍不止！
