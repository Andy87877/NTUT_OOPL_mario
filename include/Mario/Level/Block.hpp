...
class Block : public Util::GameObject {
   public:
    ...
    /**
     * Determine if this block should resolve vertical collisions first when the
     * player is grounded, to avoid incorrect horizontal snapping.
     */
    virtual bool ShouldResolveVerticallyFirst(const AABB& playerBody) const;

    /**
     * Try carrying the player if they stand on this platform.
     * Overridden by MovingPlatform.
     */
    virtual void TryCarryPlayer(PlayerState& ps, const AABB& prevAABB,
                                 bool onStaticBlock);
