...
| # | Pattern | 核心介面 / 類別 | 代碼連結 |
|---|---|---|---|
| 1 | **State**（狀態） | `ISceneHandler` + `IPlayerForm` | [ISceneHandler.hpp](include/Mario/Scenes/ISceneHandler.hpp) · [PlayerForm.hpp](include/Mario/Player/PlayerForm.hpp) |
| 2 | **Strategy**（策略） | `IEntityBehavior` | [IEntityBehavior.hpp](include/Mario/Behaviors/IEntityBehavior.hpp) |
| 3 | **MVC** | `PlayerState` / `Player` / `InputHandler` | [PlayerState.hpp](include/Mario/Player/PlayerState.hpp) · [Player.hpp](include/Mario/Player/Player.hpp) · [InputHandler.hpp](include/Mario/Services/InputHandler.hpp) |
| 4 | **Factory**（工廠） | `EntityFactory` + `BlockFactory` | [EntityFactory.hpp](include/Mario/Level/EntityFactory.hpp) · [BlockFactory.hpp](include/Mario/Level/BlockFactory.hpp) |
| 5 | **DIP**（依賴反轉） | `IAudioService` + `IInputHandler` | [IAudioService.hpp](include/Mario/Services/IAudioService.hpp) · [IInputHandler.hpp](include/Mario/Services/IInputHandler.hpp) |
| 6 | **Service Locator**（服務定位） | `ServiceLocator` | [ServiceLocator.hpp](include/Mario/Services/ServiceLocator.hpp) |
| 7 | **Facade**（外觀） | `CollisionManager` → 4 Handlers | [CollisionManager.hpp](include/Mario/CollisionManager.hpp) · [ICollisionHandler.hpp](include/Mario/Collision/ICollisionHandler.hpp) |
| 8 | **Template Method**（模板方法） | `Block::OnHit()` | [Block.hpp](include/Mario/Level/Block.hpp) |
| 9 | **Command**（命令） | `ICommand` + 12 具體命令 | [ICommand.hpp](include/Mario/Services/ICommand.hpp) · [Commands.hpp](include/Mario/Services/Commands.hpp) |
| 10 | **Registry**（登錄表） | `BehaviorRegistry` | [BehaviorRegistry.hpp](include/Mario/Level/BehaviorRegistry.hpp) |

全面採用 `std::unique_ptr` / `std::shared_ptr` **RAII** 記憶體管理，專案達到 **零 Memory Leak**。
...
1. [完整 UML 繼承圖](#1-完整-uml-繼承圖)
   ...
   - 1.16 ICommand 繼承樹 (Command Pattern — 輸入動作)
...
5. [設計模式深度解析](#5-設計模式深度解析)
   ...
   - 5.18 OCP 動態行為登錄 — BehaviorRegistry
...
classDiagram
    ...
    class IEntityBehavior {
        ...
        +OnPlayerCollision(state, player, isFromAbove, gameState, ui, camera) bool*
...
classDiagram
    ...
    class TumblePlayerDeathAnimation {
        -m_Rotation: float
        +GetRotation() float
        +GetScaleY() float
    }
    ...
    ClassicPlayerDeathAnimation <|-- TumblePlayerDeathAnimation
```
...
classDiagram
    ...
    class GameOverPanel {
        ...
        -m_FrameCount: int
        -m_Logo: UIImage
        -m_GameOverLabel: UIText
        -m_ScoreLabel: UIText
        -m_PromptLabel: UIText
    }
    class GameWonPanel {
        +Refresh(gs) void
        -m_FrameCount: int
        -m_Logo: UIImage
        -m_VictoryLabel: UIText
        -m_MarioImage: UIImage
        -m_PrincessImage: UIImage
        -m_ScoreLabel: UIText
        -m_MessageLabel: UIText
        -m_CreditLabel: UIText
        -m_PromptLabel: UIText
    }
    class ESCMenuPanel {
    ...
    IUIPanel <|.. GameOverPanel
    IUIPanel <|.. GameWonPanel
...
輸入層使用 **Command Pattern** 將「玩家按鍵動作」封裝為獨立的 `ICommand` 物件，使 `InputHandler` 僅依賴抽象命令介面，透過 `IInputProfile` 策略讀取物理按鍵後組合命令佇列執行，完全符合 OCP 與 DIP。
...
classDiagram
    ...
    class ICommand {
        <<interface>>
        +Execute(state PlayerState&, level Level&)*
    }

    class MoveRightCommand
    class MoveLeftCommand
    class StopMovingRightCommand
    class StopMovingLeftCommand
    class StopHorizontalMovementCommand
    class JumpCommand
    class CrouchCommand
    class StandUpCommand
    class RunCommand
    class StopRunningCommand
    class ShootFireballCommand
    class ApplyPhysicsMovementCommand {
        -m_Speed: float
    }
    ...
    InputHandler --> ICommand : "executes (via Execute loop)"
    InputHandler --> IInputProfile : "delegates key mapping"
```
...
| **[Service Locator](include/Mario/Services/ServiceLocator.hpp)** | 跨模組傳遞指標很麻煩 | [`ServiceLocator::GetService<T>()`](include/Mario/Services/ServiceLocator.hpp) 全域取服務 | 新增服務：Register 一次 |
| **[Facade](include/Mario/CollisionManager.hpp)** (外觀) | `CollisionManager.cpp` 曾是 800 行義大利麵 | 四個 Handler 各管一種碰撞；[`CollisionManager`](include/Mario/CollisionManager.hpp) 只負責分派；[`ICollisionHandler`](include/Mario/Collision/ICollisionHandler.hpp) 統一介面 | — |
| **[Template Method](include/Mario/Level/Block.hpp)** | Block 子類碰撞邏輯大同小異 | [`Block::OnHit()`](include/Mario/Level/Block.hpp) 固定流程 → `virtual HandleOnHit()` 留給子類 | 新增方塊：override HandleOnHit |
| **[Command](include/Mario/Services/ICommand.hpp)** (命令) | 輸入動作與 PlayerState 操作硬耦合 | [`ICommand`](include/Mario/Services/ICommand.hpp) — 12 個具體輸入命令物件 ([Commands.hpp](include/Mario/Services/Commands.hpp))；`InputHandler` 組合命令佇列，`IInputProfile` 映射物理按鍵 | 新增輸入動作：加一個 `XxxCommand` 並在 `InputHandler` 中加入 Execute 調用 |
| **[Registry](include/Mario/Level/BehaviorRegistry.hpp)** (登錄表) | `EntityFactory` 的 switch-case 每次新增敵人都需修改核心代碼 | [`BehaviorRegistry`](include/Mario/Level/BehaviorRegistry.hpp) — `unordered_map<EntityType, Creator>` 動態映射，`EntityFactory` 只呼叫 `Create()` | 新增敵人行為：只在 `BehaviorRegistry::InitializeRegistry` 加一個 lambda |
...
**原問題：** `EntityFactory::SpawnEntity()` 中使用大量 `switch(entityType)` 選擇對應的 `IEntityBehavior` 子類，每次新增敵人類型都需要修改 Factory 核心 switch 邏輯，嚴重違反了 Open-Closed Principle (OCP)。

**解法：** 導入 **BehaviorRegistry（行為登錄表）** 作為動態的 OCP 外掛點：

- `BehaviorRegistry` 內部維護一個 `std::unordered_map<EntityType, Creator>` 登錄表，其中 `Creator = std::function<std::unique_ptr<IEntityBehavior>(const EntityDef&)>`。
- 在程式啟動時，透過 `BehaviorRegistry::InitializeRegistry()` 靜態初始化，一次性將所有 EntityType 與對應的 Behavior 建立 Lambda 登錄至 map 中。
- `EntityFactory` 改呼叫 `BehaviorRegistry::Create(type, def)` 進行多型分派，完全移除了巨型 switch-case 分支。
...
classDiagram
    ...
    class BehaviorRegistry {
        <<static Registry>>
        +Create(type, def)$ unique_ptr~IEntityBehavior~
        -InitializeRegistry(map)$
    }

    class EntityFactory {
        <<static>>
        +SpawnEntity(def, x, y, dir, levelName)
    }

    EntityFactory --> BehaviorRegistry : "calls Create() — no switch"
    BehaviorRegistry --> IEntityBehavior : "constructs via lambda"
```

**成果：** 新增任何新的敵人或道具類型，只需在 `BehaviorRegistry::InitializeRegistry()` 中追加一個 lambda 條目，`EntityFactory` 核心代碼**完全無需修改**！完美實踐 Open-Closed Principle (OCP)。
...
sequenceDiagram
    ...
    else StoneBlock / InvisibleBlock / GoalBlock / WarpPipeBlock
...
sequenceDiagram
    ...
    Note over PBH: Step 1 — FallDetect
    PBH->>BCR: BodyRect(state) → full-body AABB
    PBH->>PBH: 4px strip below feet, no block → SetGrounded(false)

    Note over PBH: Step 2 — CeilingTrigger
    PBH->>PBH: narrow head hitbox → TriggerBlockHit() if hit
    ...
    loop Per active Entity with AABB overlap
        alt isFromAbove — stomp
            ...
            PEH->>PS: TakeDamage() — IPlayerForm transitions to new form
        else Item or coin
    ...
    Note over PSH: PHASE 7 — Entity-Block Collision
    ...
    loop Per active Entity
        ...
        EBH->>ES: Wall bounce → SetDirection flipped
        alt Fireball hits wall
            EBH->>EBH: ExplodesOnWall → push EXPLOSION to outNewEntities
        end
    ...
    Note over EEH: pre-filter visible cache → O(M²) not O(N²)
    loop Per visible Entity pair
        alt PlayerFireball hits Enemy
            EEH->>ES: TriggerDeath(FIREBALL)
            EEH->>EEH: delete fireball entity
        else Moving Shell hits Enemy
            EEH->>ES: TriggerDeath(SHELL)
            EEH->>EEH: add score
        end
    end
    EEH-->>CM: done
...
| `Mario/Core/FixedTimestep.hpp` | `FixedTimestep` | None (standalone) | 固定時步累積器：將實際幀時間（dtMs）切割為離散的固定物理 tick，防止不同電腦上遊戲速度波動，並設置 `maxFrameTimeMs` 硬限制以規避「死亡螺旋」。 |
...
| `Mario/Level/Block.hpp` | `Block`, `StoneBlock`, `BrickBlock`, `QuestionBlock`, `InvisibleBlock`, `GoalBlock`, `BackgroundBlock`, `BridgeBlock`, `WarpPipeBlock` | `Util::GameObject` | 磚塊基類及 9 種多型子類別；處理 hit 彈跳/破碎/物品生成邏輯及傳送水管判斷。 |
...
| `Mario/Level/Level.hpp` | `Level` | None | CSV 解析；高階 O(1) 阻擋/平台陣列；視口剔除與 visibility culling；`GetGoalBlocks()` 快取；`IsBossLevel()` 集中 8-4 識別。 |
| `Mario/Level/LevelConfig.hpp` | `LevelConfig`, `LevelPropertyProfile` | None | 關卡設定與屬性配置（含 `LevelPropertyProfile` 結構與關卡設定註冊表），消除關卡硬編碼。 |
...
| `Mario/Level/EntityAnimator.hpp` | `EntityAnimator` | None (SRP helper) | 實體動畫解析器（SRP）：將精靈路徑解析邏輯從 Entity View 類別抽離，根據 EntityState 與 EntityDef 解析精靈圖路徑，消除 Entity.cpp 的渲染職責混雜。 |
| `Mario/Level/BehaviorRegistry.hpp` | `BehaviorRegistry` | None (Registry) | OCP 動態行為登錄表：使用 `unordered_map<EntityType, Creator>` 取代 EntityFactory 內的 switch-case 分派，支援零修改擴展新敵人行為（完整 OCP）。 |
...
| `Mario/Player/PlayerDeathAnimation.hpp` | `IPlayerDeathAnimation`, `ClassicPlayerDeathAnimation`, `TumblePlayerDeathAnimation` | `IPlayerDeathAnimation` | 玩家死亡動畫多型策略（包括經典與翻轉縮小豪華動畫）。 |
| `Mario/Player/PlayerAnimator.hpp` | `PlayerAnimator` | None (SRP helper) | 玩家動畫解析器（SRP）：將精靈路徑解析邏輯從 Player View 類別抽離，提供 `GetSpritePath`、`GetSpriteState`、`GetSpriteHeight` 三方法。 |
...
| `Mario/Scenes/FlagpoleSceneHandler.hpp` | `FlagpoleSceneHandler` | `ISceneHandler` | 旗桿滑動與城堡進入過場序列。 |
...
| `Mario/Services/ICommand.hpp` | `ICommand` | None (interface) | 輸入命令抽象介面（Command Pattern）：解耦玩家輸入裝置邏輯與 PlayerState 操作動作，`Execute(state, level)` 為唯一虛擬方法。 |
| `Mario/Services/Commands.hpp` | `MoveRightCommand`, `MoveLeftCommand`, `StopMovingRightCommand`, `StopMovingLeftCommand`, `StopHorizontalMovementCommand`, `JumpCommand`, `CrouchCommand`, `StandUpCommand`, `RunCommand`, `StopRunningCommand`, `ShootFireballCommand`, `ApplyPhysicsMovementCommand` | `ICommand` | 12 個具體輸入命令實作，涵蓋移動、跳躍、蹲下、奔跑、射擊及速度物理計算等所有玩家動作。 |
...
| `Mario/UI/GameOverPanel.hpp` | `GameOverPanel` | `IUIPanel` | 專用遊戲結束文字結算面板。 |
| `Mario/UI/GameWonPanel.hpp` | `GameWonPanel` | `IUIPanel` | 專用遊戲勝利恭喜及角色謝幕面板。 |
...
以下為 project 中 51 個 C++17 實作源檔案的實際行數與職責校對：

| 檔案 | 行數 | 職責與關鍵細節說明 |
|------|-----|-------------------|
| `App.cpp` | 190 | TransitionTo + delegation to ILevelService + accessor impls；移除 Z-index 覆寫。 |
| `Mario/Core/Camera.cpp` | 55 | 8-4 Boss 鎖屏與相機橫向跟隨邏輯。 |
| `Mario/Core/PhysicsEngine.cpp` | 48 | ApplyGravity() 與 Jump 物理計算。 |
| `Mario/Core/SpritePathResolver.cpp` | 625 | 全靜態 mapping 表與 s_ResolvedPathCache 快取，避免磁碟每幀重複 I/O 開銷。 |
| `Mario/Core/FixedTimestep.cpp` | 34 | 固定時步累積器實作；`Accumulate(dtMs)` 計算本幀應執行的物理 tick 數，並防止「死亡螺旋」（超長幀時間硬截斷）。 |
| `Mario/Core/ResourceResolver.cpp` | 47 | 動態資源目錄解析（Windows 特化）；執行時優先偵測 exe 同目錄的 `./Resources`，否則回退至編譯期 `RESOURCE_DIR` 巨集，解決部署與 IDE 路徑差異問題。 |
| `Mario/Level/Block.cpp` | 400 | s_BlockSpriteCache 快取；實作 8 個 Block 子類別 HandleOnHit 多型（含 `WarpPipeBlock`）。 |
| `Mario/Level/BlockFactory.cpp` | 55 | 磚塊生成工廠，實作多型 Block 子類別建立（含水管方塊）。 |
| `Mario/Level/MovingPlatform.cpp` | 115 | 移動平台（垂直/水平）移動物理與 Snap 載人邏輯。 |
| `Mario/Level/Level.cpp` | 560 | CSV 載入與二維 Block 扁平陣列 O(1) 索引；視口 culling column 效率優化；純資料驅動食人花重疊過濾（零硬編碼）。 |
| `Mario/Player/PlayerState.cpp` | 431 | Player MVC Model；蹲下高度動態調整；自定義非內聯解構子。 |
| `Mario/Player/PlayerForm.cpp` | 304 | IPlayerForm 及 5 種力量型態子類別多型升級與傷害退化轉換實作。 |
| `Mario/Player/PlayerDeathAnimation.cpp` | 60 | ClassicPlayerDeathAnimation 死亡策略動畫（凍結➔起跳➔下墜）。 |
| `Mario/Player/PlayerAnimator.cpp` | 79 | 玩家精靈路徑解析器實作（SRP）；`GetSpritePath`/`GetSpriteState`/`GetSpriteHeight` 三方法委派至 SpritePathResolver，完全從 Player.cpp 剝離動畫邏輯。 |
| `Mario/Player/Player.cpp` | 152 | Player View；像素對齊；crouch sprite anchored to hitbox bottom（修正下陷問題）。 |
| `Mario/Services/InputHandler.cpp` | 118 | 鍵盤按鍵狀態擷取與防卡判定。 |
| `Mario/Level/EntityState.cpp` | 219 | Entity MVC Model；死亡動畫策略整合；GetHitbox 零硬編碼 AABB 運算。 |
...
| `Mario/Level/EntityAnimator.cpp` | 51 | 實體精靈路徑解析器實作（SRP）；`GetSpritePath` 委派 SpritePathResolver 並整合死亡動畫精靈鍵，從 Entity.cpp 剝離動畫路徑責任。 |
| `Mario/Level/BehaviorRegistry.cpp` | 105 | OCP 行為登錄表實作；`InitializeRegistry` 靜態初始化所有 EntityType 與 Creator lambda 映射關係，完全取代 EntityFactory 的 switch-case 分派。 |
| `Mario/Level/Entity.cpp` | 231 | Entity View；s_EntitySpriteCache 快取；Z-index 與維度由 EntityDef 資料驅動（OCP）；所有實體精靈支援底部對齊（防止漂浮）。 |
| `Mario/Level/EntityFactory.cpp` | 333 | 唯一 Entity 建立入口；設定 renderTargetWidth；SpawnProjectile 與 SpawnFromPlayer 投射物工廠。 |
| `Mario/Level/LevelConfig.cpp` | 134 | 關卡特定屬性配置（使用 `LevelPropertyProfile` 註冊表，消除所有 level 名稱 procedural `if` 判斷）。 |
| Mario/CollisionManager.cpp | 66 | **Facade門面**：公開 API 將碰撞分派給 Collision/ 目錄下的 4 個 Strategy Handler。 |
| `Mario/Collision/BlockContactResolver.cpp` | 110 | 靜態 Snap helpers（Down/Up/Right/Left）與 BodyRect 全高碰撞體建立。 |
| `Mario/Collision/PlayerBlockHandler.cpp` | 330 | 玩家-方塊三步驟物理管線：FallDetect ➔ CeilingTrigger ➔ BodyResolution。 |
| `Mario/Collision/PlayerEntityHandler.cpp` | 141 | 玩家-實體碰撞：處理踩踏 NES Combo 階梯計分與道具多型收集。 |
| `Mario/Collision/EntityBlockHandler.cpp` | 206 | 實體-方塊碰撞：處理地平/天花板 Snap/反彈/反向/落坑/火球爆炸生成；支援行為層 `IgnoresBlocks` 忽略地形；地平 snapped 精確貼合無抖動，具備速度自適應防穿透。 |
| `Mario/Collision/EntityEntityHandler.cpp` | 106 | 實體-實體碰撞：火球擊殺、龜殼踢飛；thread_local 視口快取優化由 O(N^2) 降至 O(M^2)。 |
| `Mario/Level/GameStateManager.cpp` | 140 | 核心關卡資料、生命、計時器、金幣與傳送 warp DTO 儲存。 |
| `Mario/Scenes/MenuSceneHandlers.cpp` | 169 | 標題、死亡、遊戲結束、通關場景邏輯（合併實作，減少檔案冗餘）。 |
| `Mario/Scenes/LoadingSceneHandler.cpp` | 48 | 加載畫面（預載貼圖，強制黑色背景）。 |
| `Mario/Scenes/PlayingSceneHandler.cpp` | 602 | 遊戲進行狀態 17-Phase 主迴圈與碰撞分派，旗桿觸發 DRY helper，使用多型 `IsWarpPipe()` / `IsRealAxe()` 徹底擺脫硬編碼。 |
| `Mario/Scenes/FlagpoleSceneHandler.cpp` | 206 | 旗桿滑下與城堡進入動畫過場邏輯（採用動態 AABB 貼齊，消除硬編碼）。 |
| `Mario/Scenes/PipeWarpSceneHandler.cpp` | 165 | 水管傳送過場動畫邏輯。 |
...
| `Mario/Services/AudioManager.cpp` | 273 | AudioManager 實作；音效與音樂快取讀取（DIP）。 |
| `Mario/Services/AudioPathResolver.cpp` | 9 | 音效與音樂路徑靜態解析 helper。 |
| `Mario/Services/Commands.cpp` | 127 | 12 個具體輸入命令（`ICommand`）實作，涵蓋移動/跳躍/蹲下/奔跑/射擊及物理位移計算，將輸入動作完全解耦為可獨立測試的命令物件。 |
| `Mario/Services/MockInputHandler.cpp` | 66 | 模擬/測試用輸入控制器實作。 |
| `Mario/Services/KeyboardInputProfile.cpp` | 60 | 預設鍵盤按鍵映射策略實作。 |
| `Mario/Services/LevelManager.cpp` | 155 | ILevelService 實作：LoadLevel, StartLevel, BGM 播放；StartLevel 消除 inline 旗幟尋找。 |
| `Mario/UI/HUDPanel.cpp` | 130 | HUD 面板實作，處理計分、金幣動畫更新及時間警告閃爍。 |
| `Mario/UI/TitlePanel.cpp` | 240 | 標題畫面面板實作，渲染選項列表並支援展示 controls guide 面板。 |
| `Mario/UI/TitleMenuItems.cpp` | 52 | 實作標題選單的各個具體命令類別（StartGame/Controls/QuitGame）。 |
| `Mario/UI/LoadingPanel.cpp` | 72 | 關卡加載畫面面板實作，載入並擺放角色預覽精靈。 |
| `Mario/UI/GameOverPanel.cpp` | 85 | 專用遊戲結束文字結算面板實作。 |
| `Mario/UI/GameWonPanel.cpp` | 133 | 專用遊戲勝利恭喜及角色謝幕面板實作. |
| `Mario/UI/ESCMenuPanel.cpp` | 240 | 暫停選單面板實作，渲染選項列表並支援展示 controls guide 面板。 |
| `Mario/UI/ESCMenuItems.cpp` | 145 | 實作暫停選單的各個具體命令類別（Resume/Warp/Cheat/Power/Controls）。 |
| `Mario/UI/AxeEndingPanel.cpp` | 48 | 終局城堡謝幕面板實作。 |
| `Mario/UI/CoinUI.cpp` | 86 | HUD 金幣閃爍與計數動畫實作。 |
| `Mario/UI/FloatingText.cpp` | 45 | 漂浮分數文字 60 幀 Alpha 漸進式淡出與向上漂移。 |
| `Mario/Behaviors/GoombaBehavior.cpp` | 82 | Goomba 巡邏與面牆反向、踩扁 AI。 |
| `Mario/Behaviors/KoopaFamily.cpp` | 413 | 烏龜兵、飛龜、擲斧龜多型 AI；以 KoopaType 區分，消除 string 逐幀比較。 |
| `Mario/Behaviors/BowserBehavior.cpp` | 371 | Bowser Boss 5-Phase AI（巡邏/吐火/跳躍/受傷/擊敗墜落），HP 與火球生成佇列。 |
| `Mario/Behaviors/CastleFireSpawnerBehavior.cpp` | 123 | 隱形越屏定時向左發射火球 AI。 |
| `Mario/Behaviors/FireballBehavior.cpp` | 110 | 火球拋物線與碰撞爆炸物理。 |
...
| Strategy Pattern | 19 個 `IEntityBehavior` + 4 個 `IEnemyDeathAnimation` + 2 個 `IPlayerDeathAnimation` + 5 個 `IPlayerForm` 策略 | ✅ DONE |
...
| OCP 原則 | 新增怪物/狀態/UI面板不修改現有核心類別與主迴圈；BehaviorRegistry 登錄表取代 switch-case | ✅ DONE |
| DRY 原則 | GameConfig 統一座標轉換 helpers；靜態 Sprite Cache；s_ResolvedPathCache 路徑快取 | ✅ DONE |
| Command Pattern | `ICommand` + 12 個具體輸入命令封裝玩家按鍵動作；`InputHandler` 完全解耦 | ✅ DONE |
| Registry Pattern | `BehaviorRegistry` 動態映射表取代 EntityFactory switch-case，實現 OCP 金點 | ✅ DONE |
| SRP 原則 | `PlayerAnimator`/`EntityAnimator` 從 View 剃離動畫職責；`FixedTimestep` 從 App 剃離時間累積邏輯 | ✅ DONE |
